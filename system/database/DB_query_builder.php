...
abstract class CI_DB_query_builder extends CI_DB_driver {
	...
	/**
	 * Безопасная запись данных в Базу, 
	 * с фильтрацией $_POST
	 * и определением запросов...
	 * 
	 * - разработано Нагибакой (c)
	 * 
	 */

	function _safe_insert ( $table = '' , $params = array(), $required = array(), $notInsert = array(), $owncols = array() ) 
	{
		...
		if ( FALSE !== $this->table_exists($table) ) {
				...
				$cols 		= array();
				$columns 	= array();
				$columns 	= $this->field_data($table);

				foreach ( $columns as &$_columns ) {

						$cols[$_columns->name] = $_columns;
				...
				/**
				 *  Углубленная фильтрация параметров
				 */

					$params = array_diff ( $params, array(null) );

					# Вырезаем часть запрещенных полей
					foreach ( $notInsert as $a => $b )
					{

						if ( FALSE !== ( $i = array_search( $a, array_keys ( $params ) ) ) ) 
						{
					...
					# Добавляем свои поля к запросу
					foreach ( $owncols as $x => $y )
					{
				...
				foreach ( $params as $k => $_params ) 
				{

					if ( isset( $cols[$k]->name ) && $cols[$k]->primary_key <> 1 ) 
					{
						...
						switch ( $cols[$k]->type )
						{
							...
							case 'datetime' :
								...
								$strtotimeParam = strtotime( $_params );

								$_prms[$k]      = ( ! empty ( $strtotimeParam ) ? date( 'Y-m-d H:i:s', $strtotimeParam ) : $strtotimeParam );
				...
				# Проверяем наличие обязательных полей #2

				! empty ( $_prms ) && $_prms = array_diff( $_prms, array(null) );

				foreach ( $required as $vv )
				{

					if ( ! in_array ( $vv, array_keys ( $_prms ) ) )
					{
				...
				if ( ! empty ( $_prms ) && $this->insert( $table, $_prms ) ) 
				{
	...
	/**
	 * Безопасное обновление данных в Базе, 
	 * с фильтрацией $_POST
	 * и определением запросов...
	 * 
	 * - разработано Нагибакой (c)
	...
	function _safe_update ( $table = '' , $params = array(), $where, $required = array(), $notInsert = array(), $owncols = array() ) 
	{
		...
		if ( FALSE !== $this->table_exists($table) ) 
		{
				...
				// $params = array_diff ( $params, array(null) );
					...
					foreach ( $required as $v )
					{
						...
						if ( ! in_array ( $v, array_keys ( $params ) ) || empty ( $v ) )
						{
				...
				if ( ! empty ( $_prms ) && $this->update( $table, $_prms, $where ) ) 
				{
