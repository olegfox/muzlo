...
class Auth extends EA_Admin 
{
	...
	function ext_login()
	{
		...
		$this->form_validation->set_rules('ext_login', 'ext_login', 'required');	

		if ($this->form_validation->run() == true)
		{

			! $this->ion_auth->login($this->input->post('ext_login'), '12345678', true )
	...
	function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				( $this->input->post('ref') && strpos ( $this->input->post('ref'), 'logout' ) === FALSE  ) 
				? redirect( $this->input->post('ref'), 'refresh' ) 
		...
		else
		{
			...
			$this->data['ref']	= ! empty ( $_SERVER['HTTP_REFERER'] ) ? $_SERVER['HTTP_REFERER'] : '/admin/';

					// Админка
					if ( ! $this->ion_auth->logged_in() ) {

						$this->admin_templates->assign('obj', $this->data );
						$this->admin_templates->display('login/login.tpl');

					}	else {

						$this->ion_auth->is_admin() ? redirect('/admin/', 'refresh') : redirect('/', 'refresh');
