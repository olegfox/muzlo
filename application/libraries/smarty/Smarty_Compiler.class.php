    ...
    function _compile_tag($template_tag)
    {
        ...
        switch ($tag_command) {
            ...
            case 'd':
                ...
                $str_str = explode('.',$match[3]);

                $close_debug = array( 'trimString' => 0 );

                if( isset($str_str[0]) && preg_match("/-open/i",$str_str[0]) ) {

                    $close_debug = array( 'trimString' => 0 );

                    $str_str[0] = trim(str_replace('-open','',$str_str[0]));

                }


                if( isset($str_str[0]) && preg_match("/-close/i",$str_str[0]) ) {

                    $close_debug = array( 'trimString' => 0, 'autoCollapsed' => TRUE );
                    $str_str[0] = trim(str_replace('-close','',$str_str[0]));

                }

                $pp = new nf_pp( $close_debug );
                $this->pp = &$pp;

                $str_str[0] = isset($str_str[0]) ? str_replace('$','',$str_str[0]) : FALSE;
                
                    foreach ( $str_str as $_str ) {

                        $end_str .= "['".$_str."']";

                    }

                eval('$val = $this->_tpl_vars' . $end_str . ';'); 
                return ' <div class="debug_templ"> Дамп в шаблоне: <strong>' . '/' . $this->template_dir . '/' . $this->_current_file . '</strong></div>' . 
                $this->pp->pp( $val );


            case 'pgo' :
                $this->pp = new nf_pp( array( 'trimString' => 0 ) );
                $pgo = $this->_tpl_vars;
