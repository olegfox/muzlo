/*$(function(){
 ...
 $('pre.debug_obj').not('.highlighted').html(Syntax($('pre.debug_obj').html()) ).addClass('.highlighted');
...
function Syntax(code){
	...
	// Выделяем ключевые слова
		.replace(/(var|function|typeof|Array|new|return|if|for|in|while|break|do|continue|switch|case)([^a-z0-9\$_])/gi,
			'<span class="kwrd">$1</span>$2')
	// Выделяем скобки
		.replace(/(\{|\}|\|)/gi,
			'<span class="gly">$1</span>')
	// Выделяем внутри скобок слова
		.replace(/\[(.+?)\]/gi,
			'<span class="bra">[</span><span class="set">$1</span><span class="bra">]</span>')
